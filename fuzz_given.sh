...
for i in $(find ../qa-assets-active-fuzzing/fuzz_corpora/ -mindepth 1 -maxdepth 1 -type d | grep $1);
    ...
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=1 -use_value_profile=1 -reload=1 -max_total_time=3600 -max_len=128 $i & \
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=1 -reload=1 -max_total_time=3600 -max_len=128 $i & \
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=1 -use_value_profile=1 -reload=1 -max_total_time=3600 -max_len=512 $i & \
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=1 -reload=1 -max_total_time=3600 -max_len=512 $i & \
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=1 -use_value_profile=1 -reload=1 -max_total_time=3600 -max_len=2048 $i & \
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=1 -reload=1 -max_total_time=3600 -max_len=2048 $i & \
    ...
    FUZZ=$(basename $i) build_fuzz_nosan/bin/fuzz -fork=6 -reload=1 -max_total_time=3600 $i & \
