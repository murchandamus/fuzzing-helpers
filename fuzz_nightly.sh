...
for i in $(find ../qa-assets-active-fuzzing/fuzz_corpora/ -mindepth 1 -maxdepth 1 -type d | shuf | head -n10);
    ...
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=1 -use_value_profile=1 -reload=1 -max_total_time=3600 -max_len=96 $i & \
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=1 -reload=1 -max_total_time=3600 -max_len=96 $i & \
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=1 -use_value_profile=1 -reload=1 -max_total_time=3600 -max_len=288 $i & \
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=1 -reload=1 -max_total_time=3600 -max_len=288 $i & \
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=1 -use_value_profile=1 -reload=1 -max_total_time=3600 -max_len=864 $i & \
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=1 -reload=1 -max_total_time=3600 -max_len=864 $i & \
    ...
    FUZZ=$(basename $i) build_fuzz/bin/fuzz -fork=18 -reload=1 -max_total_time=3600 $i & \
